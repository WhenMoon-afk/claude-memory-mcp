...
## 3.x – Local-only simplification

### [3.0.0] – 2026‑02‑01

#### BREAKING

- **Removed `memory_cloud` tool** — Cloud sync functionality has been removed entirely. The server is now fully local-only with zero network calls.
- **Removed `memory_connect` slash command** — No longer needed without cloud sync.

#### Removed

- Removed `src/cloud.ts` (Convex cloud sync integration)
- Removed `src/sync-manager.ts` (background sync queue with offline retry and exponential backoff)
- Removed `cloud_sync_queue` database table (schema migration 5 drops it for existing users)
- Removed `cloud_api_key` from desktop extension user config
- Removed cloud sync references from README, SKILL.md, and desktop extension docs

#### Changed

- Simplified privacy story: "Local-only. Zero telemetry." (no cloud sync opt-in language)
- Tool execution handler is now synchronous (was async only for cloud operations)
- Cleaner shutdown: no sync manager to stop

#### Migration

Existing databases will automatically run migration 5, which drops the `cloud_sync_queue` table. No data loss — only the sync metadata is removed. All memories remain intact.
