...
`mooncite serve` wraps the engine in one stdio MCP server. Codex and Claude Code connect to it directly. The packaged Pi/OMP extension only translates native tool calls to the same MCP methods; it contains no retrieval behavior.
