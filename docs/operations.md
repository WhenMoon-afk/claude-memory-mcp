...
Use the executable matching an installed older version to uninstall its package, then install 4.0.1 fresh. Mooncite safely recognizes and adopts a private, schema-identified pre-marker Mooncite index retained by 4.0.0; incompatible derived state is rebuilt from unchanged source history.
