...
Stable version numbers advance at most once per week. Tagged v4.0.5 remains the documented stable install. The current development identity and the candidate that this release will tag are `4.0.6-preview.1008.0`; do not replace stable install references without separate owner authorization for a future stable release. Untagged work is never published merely because files contain a candidate version. A candidate-version change must synchronize `src/identity.ts`, package manifests/lockfile, lifecycle expectations, and tests.
